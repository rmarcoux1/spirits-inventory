...
import type { NewBottle } from "../lib/types.js";
...
async function create(event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> {
  const body = JSON.parse(event.body ?? "{}") as NewBottle;
  if (!body.name || !body.type) {
    return jsonResponse(400, { message: "name and type are required" });
  }

  const now = new Date().toISOString();
  const item = { ...body, id: randomUUID(), created_at: now, updated_at: now };
...
async function update(id: string, event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> {
  const patch = JSON.parse(event.body ?? "{}") as Partial<NewBottle>;
